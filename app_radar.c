...
_attribute_data_retention_ static u8      radar_sta_toggle = 0;
...
#define RADAR_GIMBAL_STEP_INTERVAL_US 100000
...
void app_radar_gimbal_track_task(void)
{
    ...
    case RADAR_GIMBAL_FSM_RUN:
        ...
        if (!clock_time_exceed(g_radar_seq_step_tick, RADAR_GIMBAL_STEP_INTERVAL_US))
        {
            break;
        }
    ...
    case RADAR_GIMBAL_FSM_WAIT_DONE:
        ...
        //     break;
        // }
        // StepMotor_GimbalSetSpeedUs(1000);
        // // 停止电机
        // StepMotor_StopAll();
                                    ...
                                    float        *inout_dir_rad)
{
    ...
    for (int bounce = 0; bounce < 4; bounce++)
    {
        ...
        {
            // Fallback: no intersection found (degenerate). Clamp by keeping start.
            *out_x_mm = (s16)cur_start.x;
            *out_y_mm = (s16)cur_start.y;
            return;
        }
...
static void ReportPredictionSerialized(s16 x_mm, s16 y_mm, s16 v_cm_s)
{
    s16   dx_mm         = 0;
    s16   dy_mm         = 0;
    ...
    if (g_radar_pred.has_prev)
    {
        ...
        if ((abs(dx_mm) > STATIONARY_DXY_THRESHOLD_MM) || (abs(dy_mm) > STATIONARY_DXY_THRESHOLD_MM))
        {
            ...
            g_radar_pred.stationary_frames = 0;
        }
        else
        {
            g_radar_pred.stationary_frames++;
            if (dx_mm || dy_mm)
            {
                motion_rad = lookup_atan2((float)dx_mm, (float)dy_mm);
            }
    ...
    if (is_stationary)
    {
        ...
        BuildPredictionPoint(x_mm, y_mm, dx_mm, dy_mm, motion_rad, RadarRandRangeI32(400, 600), RadarRandRangeI32(-20, 20), &bx, &by);

        if (!radar_sta_toggle)
        {
            s16   sx      = bx;
            s16   sy      = by;
            float seq_dir = lookup_atan2((float)bx, (float)by);
        ...
        {
            RadarSeqBuildAppendPoint(ax, ay);
            RadarSeqBuildAppendPoint(bx, by);
            // LOG_D("PRED,STA,%d,%d,%d,%d", ax, ay, bx, by);
            radar_sta_toggle = 0;
        }
    }
    else
    {
        ...
        BuildPredictionPoint(x_mm, y_mm, dx_mm, dy_mm, motion_rad, RadarRandRangeI32(100, 300), RadarRandRangeI32(-10, 10), &px, &py);
...
void app_radar_parse_and_report_frame(void)
{
...

    {
        ...
        if (radar_sample_cnt >= SAMPLE_COUNT)
        {
            ...
            {
                ReportPredictionSerialized(x_in, y_in, v_avg);
            }

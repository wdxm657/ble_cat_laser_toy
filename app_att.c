...
static const u8 my_devName[] = {'W','2','M','L','a','s','e','r','T','O','Y'};
...
/* Compute CRC32 over n bytes from flash using the same crc32_half_cal as OTA library */
static u32 _ota_compute_crc(u32 base_addr, u32 n_bytes)
{
    const unsigned long tbl[16] = {
    ...
    u32 pos = 0;
    while (pos < n_bytes) {
        u32 rd = (n_bytes - pos > 256) ? 256 : (n_bytes - pos);
        flash_read_page(base_addr + pos, rd, buf);
        for (u32 i = 0; i < rd; i++) {
            nibble_buf[i * 2]     = buf[i] & 0x0F;
            nibble_buf[i * 2 + 1] = buf[i] >> 4;
        }
        crc = crc32_half_cal(crc, nibble_buf, (unsigned long *)tbl, rd * 2);
        pos += rd;
    }
